...
Listado.prototype.buscarRestaurante = function (id) {
  var restaurantEncontrado = this.restaurantes.find(function(restaurant){
...
Listado.prototype.obtenerCiudad = function () {
  ...
  var listadoDeCiudades = [];
  //Se recorre el array de restaurantes y se va agregando al array creado, todas las ubicaciones o ciudades encontradas
  listadoDeCiudades = this.restaurantes.map(function (restaurant) {
    return restaurant.ubicacion;
  })
  var nuevoListadoDeCiudades = eliminarElementosRepetidos(listadoDeCiudades);
  return nuevoListadoDeCiudades.sort();
}
...
Listado.prototype.obtenerRubros = function () {
  var listadoDeRubros = [];
  listadoDeRubros = this.restaurantes.map(function (restaurant) {
    return restaurant.rubro;
  })
  var nuevoListadoDeRubros = eliminarElementosRepetidos(listadoDeRubros);
  return nuevoListadoDeRubros.sort();
}
...
Listado.prototype.ObtenerHorarios = function () {
  ...
  arrayDeHorarios = this.restaurantes.map(function (restaurant) {
    return restaurant.horarios;
  })
  //En este arreglo vamos a poner todos los horarios, uno por uno
  var listadoDeHorarios = [];
  arrayDeHorarios.forEach(function (a) {
    a.forEach(function (hor) {
      listadoDeHorarios.push(hor)
    });
  });
  var horariosSinRepetir = eliminarElementosRepetidos(listadoDeHorarios);
  return horariosSinRepetir.sort();
}

// función que elimina los elementos repetidos de un array
function eliminarElementosRepetidos(listado) {
